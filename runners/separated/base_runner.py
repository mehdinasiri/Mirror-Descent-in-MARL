...
class Runner(object):
    def __init__(self, config):
        ...
        from algorithms.hamdpo_trainer import HAMDPO as TrainAlgo
        from algorithms.hamdpo_policy import HAMDPO_Policy as Policy
    ...
    def train(self):
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            self.trainer[agent_id].kl_lr = self.kl_lr
    ...
    def log_env(self, env_infos, total_num_steps):
        for k, v in env_infos.items():
            if len(v) > 0:
                self.writter.add_scalar(f"env_infos/{k}", np.mean(v), total_num_steps)
